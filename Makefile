...
# --parallelism 1: keeps peak memory within the CI runner (matches the family's
# other exporters, which serialize multi-arch builds for the same reason).
release:
...
# --parallelism 1 matches `release:` — protects memory-constrained local boxes / devcontainers.
release-snapshot:
